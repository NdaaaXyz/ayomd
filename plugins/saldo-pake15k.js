let handler = async (m, { conn, text, usedPrefix, args, participants }) => {
 ...
 if (user.saldo === 0) throw `Kamu sudah melebihi token/limit memasukkan bot ke dalam group!`

  var anubot = owner[0]

  m.reply(`Tunggu 3 detik bot akan join`)
  global.db.data.users[m.sender]. saldo -= 15000
...
handler.help = ['pakesaldo15k <chat.whatsapp.com>']

handler.tags = ['saldo']

handler.command = /^(pakesaldo15k)$/i
