let handler = async (m, { conn, text, usedPrefix, args, participants }) => {
  ...
  if (user.saldo === 0) throw `Kamu sudah tidak punya saldo lagi memasukkan bot ke dalam group!\n untuk mengisi chat lah owner`

  var anubot = owner[0]

  m.reply(`Tunggu 3 detik bot akan join`)

  global.db.data.users[m.sender]. saldo -= 25000
...
handler.help = ['pakesaldo25k <chat.whatsapp.com>']

handler.tags = ['saldo']

handler.command = /^(pakesaldo25k|usesaldo25k)$/i
