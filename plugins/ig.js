...
let handler = async (m, { conn, args, usedPrefix, command }) => {
    ...
    for (const { url } of results) await conn.sendFile(m.chat, url, 'instagram.mp4', `馃敆 *Url:* ${await shortlink(url)}\n*${global.wm}*`, m)
...
let handler = async (m, { conn, args, usedPrefix, command }) => {
  ...
  m.reply('Sedang diproses...')
   ...
   conn.sendFile(m.chat, json.linkdownload, 'ig.mp4', '*漏 Aine*', m, { thumbnail: Buffer.alloc(0) })
