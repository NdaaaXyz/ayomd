...
let handler = async (m, { conn, args, usedPrefix, isOwner }) => {
    ...
    if (m.chat in conn.judi) return m.reply ('Masih ada yang melakukan reme disini, tunggu sampai selesai!!')
    ...
    try {
        ...
        if (args.length < 1) return conn.reply(m.chat, usedPrefix + 'reme <jumlah>\n ' + usedPrefix + 'judi 1000', m)
        if (global.db.data.users[m.sender].money >= count * 1) {
            global.db.data.users[m.sender].money -= count * 1
            await m.reply('*Jangan main reme gk bakal menang!!, kalau gk percaya gpp*') //Kwkwwkkwlwlw
            if (Aku > Kamu) {
                ...
                conn.reply(m.chat, `aku roll:${Aku}\nKamu roll: ${Kamu}\n\nkamu baru saja *menang* melawan richboy999, dan kamu Mendapatkan ${count * 2} money`.trim(), m)
            } else {
                global.db.data.users[m.sender].money += count * 1
                conn.reply(m.chat, `aku roll:${Aku}\nKamu roll: ${Kamu}\n\nkamu baru saja *seri* melawan richboy999,dan kamu Mendapatkan ${count * 1} money`.trim(), m)
            }
